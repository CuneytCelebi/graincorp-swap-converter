...
import pandas as pd
# nn Page config nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
st.set_page_config(
...
# nn Config nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
HEADERS_ROW = [
...
Rules:
- swap_no: the SWAP NO number only e.g. "6001086404"
- transfer_date: TRANSFER DATE in DD/MM/YYYY format e.g. "23/01/2026"
- from_site_code: the numeric Code value from the STOCK SWAP SITE FROM table e.g. "5168"
- to_site_code: the numeric Code value from the STOCK SWAP TO SITE table e.g. "4207"
- from_site_name: format as "SITECODE SITENAME GC" using the actual code and site name from the FROM table e.g. "5168 MIRROOL
GC" — use the actual numeric code from the document, NOT "2203"
- to_site_name: format as "SITECODE SITENAME GC" using the actual code and site name from the TO table e.g. "4207 RED BEND GC" —
use the actual numeric code from the document, NOT "2203"
- rate_from: the Freight dollar amount from STOCK SWAP SITE FROM as a number e.g. 51.20
- rate_to: the Freight dollar amount from STOCK SWAP TO SITE as a number e.g. 56.52
- grade: the GRADE field e.g. "H2" or "APW1"
- season: 4-digit integer from SEASON field, slash removed e.g. "25/26" becomes 2526
- tonnes: the Tonnes value as a number e.g. 1688.51
Return ONLY the JSON object, nothing else."""
# nn Session state nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
...
# nn Rate table helpers nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
...
return mismatches
# nn Gemini helpers nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
...
# nn Excel builder nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
# Highlight colours
...
<div style='color:white;font-size:20px;font-weight:700;'>n GC</div>
<div style='color:#A8D5B5;font-size:12px;margin-top:4px;'>Swap Converter</div>
</div>""", unsafe_allow_html=True)
st.markdown("### n API Key")
api_key = st.text_input("Google Gemini API Key", type="password",
placeholder="AIzaSy...",
help="Get a free key at aistudio.google.com")
if api_key:
st.success("3 API key entered")
else:
...
if total_rates:
st.success(f"3 Rate table loaded — {total_rates} sites")
else:
st.warning("nn No rate table loaded.\nUpload one in the **Rates** tab.")
st.divider()
...
st.success(f"n Loaded {len(rate_map)} site(s) from **{rate_file.name}**")
add_log(f"Rate table loaded: {len(rate_map)} sites from {rate_file.name}", "ok")
else:
st.error("n Could not find required columns. "
"Make sure your file has: site_code, rate_from, rate_to")
except Exception as e:
st.error(f"n Error reading file: {e}")
...
if st.button("nn Clear Rate Table"):
...
<div class='metric-val'>{'3' if rate_table else '—'}</div>
...
if not rate_table:
st.warning("nn No rate table loaded — rates won't be validated. "
"Upload one in the **Rate Table** tab to enable mismatch highlighting.")
st.markdown("<br>", unsafe_allow_html=True)
st.markdown("#### n Upload Swap PDFs")
...
st.markdown("<br>", unsafe_allow_html=True)
if not api_key:
st.warning("nn Please enter your Gemini API key in the sidebar first.")
else:
if st.button("n Extract All & Download Excel",
...
add_log(f"n Swap {swap_no}: {len(mismatches)} rate mismatch(es)", "warn")
else:
add_log(f"3 Swap {swap_no} extracted, rates OK [{model}]", "ok")
results.append(data)
...
add_log(f"7 Failed: {f.name} — {e}", "error")
...
if total_mm > 0:
status.warning(f"nn Extracted {len(results)} record(s) — "
f"**{total_mm} rate mismatch(es) found** "
f"(highlighted in Excel)")
else:
status.success(f"n Extracted {len(results)} record(s) — "
f"all rates match!")
if errors:
st.warning(f"nn {len(errors)} file(s) failed: {', '.join(errors)}")
# nn Mismatch summary nn
if total_mm > 0:
st.markdown("#### nn Rate Mismatches Found")
...
# nn Data preview nn
with st.expander("nn Preview extracted data", expanded=False):
for r in results:
swap_no = str(r.get("swap_no",""))
...
st.download_button("n Export CSV", buf.getvalue(),
...
mm_badge = (f"<span class='badge-warn'>n {mm_count} mismatch(es)</span>"
if mm_count else "<span class='badge-ok'>3 OK</span>")
rows_html += f"""<tr>
